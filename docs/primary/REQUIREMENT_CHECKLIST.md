...
- [`docs/primary/SCIENTIFIC_WORKFLOW.md`](SCIENTIFIC_WORKFLOW.md) is the scientific protocol.
- [`docs/CODEBASE_GUIDE.md`](../CODEBASE_GUIDE.md) is the implementation and operations
  guide.
...
| Seven detectors including LRR | `experiment_core/detectors/scoring.py`, `experiment_core/analysis/evaluation.py` | `tests/test_scoring.py`, `tests/test_evaluation.py` |
...
| Restart, completed keys, partial line repair, completion markers | `experiment_core/infrastructure/io.py`, pipeline markers | `tests/test_io.py` |
...
| Isolated one-GPU Qwen 0.5B gate | `configs/delta_smoke_qwen_0_5b.json`, `tools/delta/delta_smoke_qwen_0_5b.sbatch`, `tools/delta/submit_delta_smoke.sh`, `smoke_validation.py` | Synthetic two-pass run and static resource/account tests pass; real Slurm/CUDA execution awaiting Delta |
...
| Retain current Delta workflow and canonical plotter | `docs/delta/DELTA.md`, `docs/delta/DELTA_SMOKE.md`, current `tools/delta/` launchers, configs, and `plot_tpr_contamination.py` | Static Delta tests and synthetic smoke pass |
| Canonical scientific documentation | `docs/primary/SCIENTIFIC_WORKFLOW.md`, linked prominently from `README.md` | Covers the 21 unique cells, frozen settings, leakage rules, bootstrap, storage, result classes, risks, external freezing, and invalidation matrix |
| Canonical codebase documentation | `docs/CODEBASE_GUIDE.md`, linked prominently from `README.md` | Covers current tree, entry points, functions, I/O, precomputation, resume, plotting, Delta, extensions, tests, and limitations |
| Residual obsolete tracked artifacts | Retained toy/old plotting sources plus `.DS_Store`, tracked bytecode, and historical PNGs remain inactive | Explicitly identified in `docs/CODEBASE_GUIDE.md`; they are not linked as active workflow or results |
...
throughput/peak GPU memory. Follow the gate in `docs/delta/DELTA.md`; do not mark paper runs
