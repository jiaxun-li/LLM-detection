...
The Slurm job file is [`tools/delta/delta_experiment.sbatch`](../../tools/delta/delta_experiment.sbatch).
...
sbatch \
  ...
  tools/delta/delta_experiment.sbatch
```
...
ACCOUNT=<delta-account> EXPERIMENT_SET=primary \
  bash tools/delta/submit_delta_matrix.sh
```
